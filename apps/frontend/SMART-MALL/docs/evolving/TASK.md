...
| P3   | 领域场景层 (Domain Layer) | 18     | 5-7h     | 高     | 已完成 (100%)    |
...
| **PX** | **商城建模器 (Mall Builder)** | **25** | **8-10h** | **高** | **已完成 (100%)** |

**总计**: 约 173 个任务，预估 53-66 小时
...
- [X] 3.3.1 实现导航行为 (src/domain/behaviors/NavigationBehavior.ts)

  - navigateToStore(storeId): DomainResult
  - navigateToArea(areaId): DomainResult
  - 计算目标位置和相机角度
  - _Requirements: 12.1, 12.2, 12.3_
  - **已完成**: 实现了 NavigationBehavior 类，支持平滑相机动画导航
- [X] 3.3.2 实现高亮行为 (src/domain/behaviors/HighlightBehavior.ts)

  - highlightStore(storeId): DomainResult
  - clearHighlight(): DomainResult
  - 管理高亮状态
  - _Requirements: 12.4, 12.5, 12.6_
  - **已完成**: 实现了 HighlightBehavior 类，连接语义层和渲染层
- [X] 3.3.3 实现楼层切换行为 (src/domain/behaviors/FloorSwitchBehavior.ts)

  - switchFloor(floorId): DomainResult
  - 隐藏当前楼层，显示目标楼层
  - 过渡动画
  - _Requirements: 13.1, 13.2, 13.3, 13.6_
  - **设计变更**: 改为连续空间导航，已合并到 NavigationBehavior.navigateToFloor()
- [X] 3.3.4 实现场景查询行为 (src/domain/behaviors/SceneQueryBehavior.ts)

  - getStoreById(id): Store | null
  - getStoresByArea(areaId): Store[]
  - getFloorStores(floorId): Store[]
  - _Requirements: 2.5_
  - **已完成**: 实现了 SceneQueryBehavior 类，提供统一的场景查询接口

### 3.4 领域事件处理

- [X] 3.4.1 创建 DomainEventHandler 类 (src/domain/events/DomainEventHandler.ts)

  - 将 Three Core 的场景事件转换为领域事件
  - scene.click → store.selected
  - scene.hover → store.focused
  - _Requirements: 11.5, 11.6_
  - **已完成**: 实现了 DomainEventHandler 类，支持事件翻译
- [X] 3.4.2 实现领域事件分发

  - 事件订阅/取消订阅
  - 事件广播
  - _Requirements: PROTOCOL.md 5.2_
  - **已完成**: 实现了 DomainEventBus 类，支持发布-订阅模式

### 3.5 数据加载与验证

- [X] 3.5.1 创建 MallDataLoader 类 (src/domain/loader/MallDataLoader.ts)

  - 从 API 或配置文件加载商城数据
  - 数据格式转换
  - _Requirements: 9.1, 9.2_
  - **已完成**: 实现了 MallDataLoader 类，支持 API/配置/JSON 加载
- [X] 3.5.2 创建 MallDataValidator 类 (src/domain/loader/MallDataValidator.ts)

  - 验证数据结构完整性
  - 验证必填字段
  - 验证引用关系
  - _Requirements: 17.1, 17.2, 17.3, 17.4, 17.5, 17.6_
  - **已完成**: 实现了 MallDataValidator 类，支持完整的数据验证

### 3.6 Domain Layer 检查点

- [X] 3.6.0 Checkpoint - 确保领域层完成
  - 确保语义对象可以正确创建和查询
  - 确保导航、高亮、楼层切换功能正常
  - 确保数据加载和验证正常
  - _Requirements: 需求 2, 9, 12, 13_
  - **已完成**: 创建了 DomainTestView.vue 集成测试页面，访问 /domain-test 验证
...
## PX. 商城建模器 (Mall Builder) ✅ 已完成

> 目标：实现商城 3D 建模功能，支持管理员创建和编辑商城布局
>
> **状态**: 已完成 (2026-01-02)
...
- [X] X.3.1 矩形商城模板（100x80，3层）
- [X] X.3.2 L形商城模板（120x100，3层）
- [X] X.3.3 U形商城模板（120x100，3层）
- [X] X.3.4 T形商城模板（120x100，3层）
- [X] X.3.5 圆形商城模板（半径50，32边形近似，3层）
- [X] X.3.6 所有模板轮廓居中生成

### X.4 几何计算模块

- [X] X.4.1 多边形面积计算（Shoelace 公式）
- [X] X.4.2 多边形周长计算
- [X] X.4.3 点在多边形内检测（射线法）
- [X] X.4.4 多边形包含检测
- [X] X.4.5 多边形重叠检测（SAT 分离轴定理）
- [X] X.4.6 网格对齐（snapToGrid）

### X.5 工具栏

- [X] X.5.1 选择工具（V）- 点击选中区域
- [X] X.5.2 平移工具 - 启用相机旋转/缩放/平移
- [X] X.5.3 矩形绘制工具（R）
- [X] X.5.4 多边形绘制工具（P）
- [X] X.5.5 轮廓绘制工具
- [X] X.5.6 重置轮廓工具
...
- [X] X.6.1 深色主题设计（与登录页一致）
- [X] X.6.2 毛玻璃效果面板
- [X] X.6.3 SVG 图标（无 emoji）
- [X] X.6.4 快捷键支持（V/R/P/Del/Ctrl+Z/Ctrl+S/Esc）
- [X] X.6.5 底部状态栏（当前楼层、区域数量、快捷键提示）
- [X] X.6.6 帮助面板（使用指南）
- [X] X.6.7 场景说明图例（左下角浮动面板）

### X.7 历史记录与导出

- [X] X.7.1 历史记录（撤销/重做，最多50步）
- [X] X.7.2 项目导出（JSON格式）
- [X] X.7.3 项目导入
- [X] X.7.4 背景图片导入（参考图辅助绘制）

### X.8 属性测试

- [X] X.8.1 多边形面积计算属性测试
- [X] X.8.2 多边形周长计算属性测试
- [X] X.8.3 点在多边形内检测属性测试
- [X] X.8.4 模板生成属性测试
- [X] X.8.5 绘图工具状态机属性测试
- [X] X.8.6 背景图片状态管理属性测试
- [X] X.8.7 3D 渲染函数属性测试

### X.9 Mall Builder 检查点

- [X] X.9.0 Checkpoint - 确保建模器完成
  - ✅ 项目创建向导正常
  - ✅ 楼层管理功能正常
  - ✅ 区域绘制功能正常
  - ✅ 3D 渲染正常
