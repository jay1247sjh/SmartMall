<!--
  ============================================================================
  BuilderView.vue - 商城建模器页面
  ============================================================================

  【页面职责】
  提供 3D 建模环境，让管理员可以创建和编辑商城的 3D 场景。
  这是智慧商城系统的核心功能页面之一。

  【业务背景】
  商城建模器允许管理员：
  - 在 3D 空间中放置店铺、区域、设施等对象
  - 调整对象的位置、大小、旋转
  - 划分楼层和区域
  - 保存和加载商城布局
  ...
  【技术实现】
  - 使用 ThreeEngine 进行 3D 渲染
  - 使用 RaycasterManager 进行射线检测（点击检测）
  - 使用 OrbitController 进行相机控制

  【与其他模块的关系】
  - engine/ThreeEngine.ts：3D 渲染引擎
  - stores/builder.store.ts：建模器状态管理
  - api/mall-builder.api.ts：保存/加载商城布局
