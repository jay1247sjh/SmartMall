/**
 * ============================================================================
 * 商城状态 Store (Mall Store)
 * ============================================================================
 *
 * 【业务职责】
 * 管理 Smart Mall 的核心业务数据——商城结构。这是整个 3D 商城的数据基础，
 * 所有的可视化展示、交互操作都依赖此 Store 提供的数据。
 *
 * 【数据层级结构】
 * Smart Mall 采用四级数据结构，模拟真实商场的物理布局：
 *
 *   Mall（商城）
 *     └── Floor（楼层）
 *           └── Area（区域/铺位）
 ...
 * 1. Mall（商城）
 *    - 最顶层实体，代表一个完整的商业综合体
 *    - 包含商城名称、描述、状态、布局版本等元信息
 *    - 一个系统可以管理多个商城（多租户架构预留）
 *
 * 2. Floor（楼层）
 *    - 商城的垂直分层，如 B1、1F、2F 等
 *    - 包含楼层高度、3D 位置等几何信息
 *    - 用户可以在不同楼层间切换浏览
 *
 * 3. Area（区域/铺位）
 ...
 *   PENDING（审批中）
 *      ↓ 管理员审批通过        ↘ 管理员拒绝
 *   AUTHORIZED（已授权）      LOCKED（锁定）
 *      ↓ 商家完成店铺建模
 *   OCCUPIED（已入驻）
 *
 * 【选中状态管理】
 * 维护用户在 3D 场景中的选中状态：
 * - currentFloorId：当前查看的楼层
 * - selectedAreaId：当前选中的区域（高亮显示）
 * - selectedStoreId：当前选中的店铺（显示详情）
 *
 * 【与 3D 引擎的关系】
 * 此 Store 提供数据，3D 引擎负责渲染：
 * - Store 数据变化 → 触发 3D 场景更新
 * - 3D 场景交互 → 更新 Store 选中状态
 * - 实现数据与视图的双向同步
 * ============================================================================
 */
